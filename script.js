...
function showPage(pageId) {
    ...
    switch (pageId) {
        ...
        case 'lojas': renderizarProdutosLojas(); break; // Chama a nova função
...
function salvarCotacao() {
    if (!cotacaoAtual) { mostrarNotificacao('Gere resumo!', 'error'); return; }
...
/**
 * REFEITO (Layout de Card Simplificado):
 * Renderiza um card por produto com um controle de lucro "global"
 * que recalcula uma lista de preços de lojas.
 */
function renderizarProdutosLojas() {
    ...
    itensFiltrados.forEach(item => {
        ...
        const idBase = `${item.id}-${item.isKit}`; // ID único para o card

        // Card principal
        let cardHTML = `<div class="precificacao-card">`;

        // --- 1. Header do Card (Info do Produto) ---
        cardHTML += `
            <div class="precificacao-header">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-gray-400">SKU: ${item.sku} | Custo Total: <span class="font-bold text-red-400">${formatarMoeda(custoTotal)}</span></p>
                </div>
            </div>
        `;

        // --- 2. Controles do Card (O Input de Lucro) ---
        cardHTML += `
        ...
        // --- 3. Lista de Lojas (Onde os preços aparecem) ---
        ...
        Object.keys(lojasConfig).forEach(key => {
            ...
            cardHTML += `
                <div class="precificacao-loja-item">
                    ...
                    <span class="loja-preco-sugerido" id="preco-${key}-${idBase}">R$ 0,00</span>
                </div>
            `;
        });
...
/**
 * NOVO: Recalcula TODOS os preços de um card
 * baseado no input de "Lucro desejado" global do card.
 */
function recalcularCardProduto(itemId, isKit) {
    ...
    // 2. Definir o ID base para este card
    const idBase = `${itemId}-${isKit}`;

    // 3. Pegar o Custo Total
    const custoTotal = isKit ? item.custoTotal : (item.custo + item.picking);

    // 4. Pegar o Lucro Desejado do input global do card
    const lucroInput = document.getElementById(`lucro-global-${idBase}`);
    if (!lucroInput) return; // Se o elemento ainda não foi renderizado
    const lucroDesejado = parseFloat(lucroInput.value) || 0;

    // 5. Variáveis para calcular a média
    let totalMargem = 0;
    let totalLojas = 0;

    // 6. Loop em CADA loja para calcular e atualizar o preço
    Object.keys(lojasConfig).forEach(lojaKey => {
        const cfg = lojasConfig[lojaKey]; // Config da loja (taxas)

        // --- O CÁLCULO ---
        let precoSugerido = 0;
        const divisor = 1 - cfg.comissao;
        
        if (divisor > 0) { // Lojas com comissão %
            precoSugerido = (custoTotal + cfg.taxaFixa + lucroDesejado) / divisor;
        } else { // Lojas sem comissão (Face/Wpp)
            precoSugerido = custoTotal + cfg.taxaFixa + lucroDesejado;
        }

        // Calcular lucro real e margem para esta loja
        ...
        totalMargem += margemReal; // Adiciona à soma da média
        totalLojas++;

        // 7. Atualizar o preço na tela
        const precoSpan = document.getElementById(`preco-${lojaKey}-${idBase}`);
        if (precoSpan) {
            precoSpan.textContent = formatarMoeda(precoSugerido);
            // Adiciona cor se o lucro for negativo
    ...
    const margemMedia = totalMargem / totalLojas;
    const margemMediaSpan = document.getElementById(`margem-media-${idBase}`);
    if (margemMediaSpan) {
        margemMediaSpan.textContent = `${margemMedia.toFixed(1)}%`;
    }
}
